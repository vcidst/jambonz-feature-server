...
router.post('/', async(req, res) => {
  ...
  try {
    ...
    if (target.type === 'phone' && !target.trunk) {
      ...
      const voip_carrier_sid = await lookupTrunk(req.body.account_sid, target.number);
      logger.info(
        `createCall: selected ${voip_carrier_sid} for requested phone number: ${target.number || 'unspecified'})`);
      if (voip_carrier_sid) {
        opts.headers['X-Requested-Carrier-Sid'] = voip_carrier_sid;
      }
    }
