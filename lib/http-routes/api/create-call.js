...
router.post('/', async(req, res) => {
  ...
  try {
     ...
     * check if number matches any of our trunks
     *  */
    if (target.type === 'phone' && !target.trunk) {
      const {lookupTrunk} = dbUtils(this.logger, srf);
      const trunk = await lookupTrunk(req.body.account_sid, target.number);
      if (trunk) {
        logger.info(
          `createCall: selected ${trunk.voip_carrier_sid} for requested phone number: ${trunk.number || 'unspecified'})`);
      }
      if (trunk.voip_carrier_sid) {
        opts.headers['X-Requested-Carrier-Sid'] = trunk.voip_carrier_sid;
      }
    }
